CREATE TABLE [dbo].[DELTA_LOG_LOGRADOURO] (
    ...
    [LOG_OPERACAO]    CHAR (3)      NULL,
    [CEP_ANT]         CHAR (8)      NULL,
    CONSTRAINT [PK_DELTA_LOG_LOGRADOURO] PRIMARY KEY CLUSTERED ([LOG_NU] ASC)
);


GO
CREATE TRIGGER [TrgLog_DELTA_LOG_LOGRADOURO] ON [Implanta_CRPAM].[dbo].[DELTA_LOG_LOGRADOURO] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'DELTA_LOG_LOGRADOURO'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
	SET @TipoOperacao = 'Alteração' 
 	SELECT @Conteudo = 'LOG_NU : «' + RTRIM( ISNULL( CAST (LOG_NU AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| LOG_NO_ABREV : «' + RTRIM( ISNULL( CAST (LOG_NO_ABREV AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| LOG_OPERACAO : «' + RTRIM( ISNULL( CAST (LOG_OPERACAO AS VARCHAR(3500)),'Nulo'))+'» '
