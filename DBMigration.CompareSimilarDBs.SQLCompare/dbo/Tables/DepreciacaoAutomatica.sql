CREATE TABLE [dbo].[DepreciacaoAutomatica] (
    ...
    [Exercicio]               INT              NULL,
    [IdTipoBem]               INT              NULL,
    CONSTRAINT [PK_DepreciacaoAutomatica] PRIMARY KEY CLUSTERED ([IdDepreciacaoAutomatica] ASC),
    CONSTRAINT [FK_DepreciacaoAutomatica_TiposBens] FOREIGN KEY ([IdTipoBem]) REFERENCES [dbo].[TiposBens] ([IdTipo])
);


GO
CREATE TRIGGER [TrgLog_DepreciacaoAutomatica] ON [Implanta_CRPAM].[dbo].[DepreciacaoAutomatica] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'DepreciacaoAutomatica'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
