CREATE TABLE [dbo].[LOG_GRANDE_USUARIO] (
    [GRU_NU]       NUMERIC (8)   NULL,
    [UFE_SG]       CHAR (2)      NULL,
...
GO
CREATE TRIGGER [TrgLog_LOG_GRANDE_USUARIO] ON [Implanta_CRPAM].[dbo].[LOG_GRANDE_USUARIO] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'LOG_GRANDE_USUARIO'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
	SET @TipoOperacao = 'Alteração' 
 	SELECT @Conteudo = 'GRU_NU : «' + RTRIM( ISNULL( CAST (GRU_NU AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| GRU_NO_ABREV : «' + RTRIM( ISNULL( CAST (GRU_NO_ABREV AS VARCHAR(3500)),'Nulo'))+'» ' FROM DELETED 
	SELECT @Conteudo2 = 'GRU_NU : «' + RTRIM( ISNULL( CAST (GRU_NU AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| GRU_ENDERECO : «' + RTRIM( ISNULL( CAST (GRU_ENDERECO AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| CEP : «' + RTRIM( ISNULL( CAST (CEP AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| GRU_NO_ABREV : «' + RTRIM( ISNULL( CAST (GRU_NO_ABREV AS VARCHAR(3500)),'Nulo'))+'» ' FROM INSERTED 
