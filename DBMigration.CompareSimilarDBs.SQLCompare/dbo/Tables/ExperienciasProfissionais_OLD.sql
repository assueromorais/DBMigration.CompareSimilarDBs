CREATE TABLE [dbo].[ExperienciasProfissionais_OLD] (
...
GO
CREATE TRIGGER [TrgLog_ExperienciasProfissionais_OLD] ON [Implanta_CRPAM].[dbo].[ExperienciasProfissionais_OLD] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'ExperienciasProfissionais_OLD'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
	SET @TipoOperacao = 'Alteração' 
 	SELECT @Conteudo = 'IdExperienciaProfissional : «' + RTRIM( ISNULL( CAST (IdExperienciaProfissional AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| IdCidadeAtuacao : «' + RTRIM( ISNULL( CAST (IdCidadeAtuacao AS VARCHAR(3500)),'Nulo'))+'» ' FROM DELETED 
	SELECT @Conteudo2 = 'IdExperienciaProfissional : «' + RTRIM( ISNULL( CAST (IdExperienciaProfissional AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| IdProfissional : «' + RTRIM( ISNULL( CAST (IdProfissional AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| IdPessoaJuridica : «' + RTRIM( ISNULL( CAST (IdPessoaJuridica AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| IdCidadeAtuacao : «' + RTRIM( ISNULL( CAST (IdCidadeAtuacao AS VARCHAR(3500)),'Nulo'))+'» ' FROM INSERTED 
