CREATE TABLE [dbo].[DepreciacaoAutomaticaMCASP] (
    ...
    PRIMARY KEY CLUSTERED ([IdDAMCASP] ASC),
    FOREIGN KEY ([IdDepreciacaoAutomatica]) REFERENCES [dbo].[DepreciacaoAutomatica] ([IdDepreciacaoAutomatica]),
    FOREIGN KEY ([IdItem]) REFERENCES [dbo].[ItensMoveis] ([IdItem])
);


GO
CREATE TRIGGER [TrgLog_DepreciacaoAutomaticaMCASP] ON [Implanta_CRPAM].[dbo].[DepreciacaoAutomaticaMCASP] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'DepreciacaoAutomaticaMCASP'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
