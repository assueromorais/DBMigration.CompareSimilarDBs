CREATE TABLE [dbo].[DominiosSiscafwProcessos] (
    [IdSistema]        INT           NULL,
    ...
    [Acesso]           INT           NULL,
    [CampoJoin]        VARCHAR (200) NULL
);


GO
CREATE TRIGGER [TrgLog_DominiosSiscafwProcessos] ON [Implanta_CRPAM].[dbo].[DominiosSiscafwProcessos] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'DominiosSiscafwProcessos'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
	SET @TipoOperacao = 'Alteração' 
 	SELECT @Conteudo = 'IdSistema : «' + RTRIM( ISNULL( CAST (IdSistema AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| CampoJoin : «' + RTRIM( ISNULL( CAST (CampoJoin AS VARCHAR(3500)),'Nulo'))+'» ' FROM DELETED 
	SELECT @Conteudo2 = 'IdSistema : «' + RTRIM( ISNULL( CAST (IdSistema AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| Ordem : «' + RTRIM( ISNULL( CAST (Ordem AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| Acesso : «' + RTRIM( ISNULL( CAST (Acesso AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| CampoJoin : «' + RTRIM( ISNULL( CAST (CampoJoin AS VARCHAR(3500)),'Nulo'))+'» ' FROM INSERTED 
