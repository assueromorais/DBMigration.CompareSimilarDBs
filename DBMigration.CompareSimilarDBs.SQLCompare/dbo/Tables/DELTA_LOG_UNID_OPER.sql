CREATE TABLE [dbo].[DELTA_LOG_UNID_OPER] (
    ...
    [UOP_OPERACAO] CHAR (3)      NULL,
    [CEP_ANT]      CHAR (8)      NULL,
    CONSTRAINT [PK_DELTA_LOG_UNID_OPER] PRIMARY KEY CLUSTERED ([UOP_NU] ASC)
);


GO
CREATE TRIGGER [TrgLog_DELTA_LOG_UNID_OPER] ON [Implanta_CRPAM].[dbo].[DELTA_LOG_UNID_OPER] 
...
SET @Conteudo = Null 
SET @Conteudo2 = Null 
SET @TableName = 'DELTA_LOG_UNID_OPER'
IF   ( @CountI   =   1  )  AND 
     ( @CountD   =   1  ) 
BEGIN 
	SET @TipoOperacao = 'Alteração' 
 	SELECT @Conteudo = 'UOP_NU : «' + RTRIM( ISNULL( CAST (UOP_NU AS VARCHAR(3500)),'Nulo'))+'» '
                         ...
                         + '| UOP_NO_ABREV : «' + RTRIM( ISNULL( CAST (UOP_NO_ABREV AS VARCHAR(3500)),'Nulo'))+'» '
                         + '| UOP_OPERACAO : «' + RTRIM( ISNULL( CAST (UOP_OPERACAO AS VARCHAR(3500)),'Nulo'))+'» '
