/*Kleber - 14/01/2011 - OC 72668 - Foi colocado um "distinct" na consulta final */
/* OC 17219 - Rodrigo Souza */ 


CREATE PROCEDURE [dbo].[Sp_IntegraSiproAgenda]  
@Tipo char(1) = '',  
@TpConta VARCHAR(50) = '',
...
DECLARE @IdContaFinanceira int, @NomeContaFinanceira varchar(60), @CodigoContaFinanceira varchar(60), @TipoConta char(1), @IdConta int,  
   ...
   @CodPai varchar(20), @Sequencia varchar(3)  
...
EXEC('SELECT distinct dbo.PlanoContas.CodConta, dbo.PlanoContas.NomeConta,dbo.PlanoContasFinanceiro.NomeContaFinanceira,   
dbo.PlanoContasIntegracao.IdConta, dbo.PlanoContasIntegracao.IdContaFinanceira,#PlContaFin.CodigoContaFinanceira,  
dbo.PlanoContasFinanceiro.TipoConta  
FROM #PlContaFin,dbo.PlanoContasIntegracao,dbo.PlanoContasFinanceiro,dbo.PlanoContas  
where  dbo.PlanoContas.IdConta = dbo.PlanoContasIntegracao.IdConta and  
       dbo.PlanoContasIntegracao.IdContaFinanceira = dbo.PlanoContasFinanceiro.IdContaFinanceira  
and dbo.PlanoContasIntegracao.IdContaFinanceira = #PlContaFin.IdContaFinanceira  
and dbo.PlanoContasFinanceiro.TipoConta IN (' + @TpConta + ')')
