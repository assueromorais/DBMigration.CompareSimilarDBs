...
CREATE PROCEDURE dbo.sp_CA_AtualizaDebitos 
/*select * from moedas
  select * from ProcedimentosAtraso
  select * from sequenciasCalculos
  select * from TiposDebito*/
...
@lChamadaInterna int,
@Total float ,
@RetornoMulta float OUTPUT,
@RetornoAtualizacao float OUTPUT,
@RetornoValorTotal float OUTPUT

AS

SET NOCOUNT ON
 DECLARE @NumSequencia int, @DataDe DateTime, @DataAte DateTime, @lValorFixo bit, @ValorFixo money, 
	    @RotinaCalculo int, @PercentMultaJuros float, @AplicarSeq varchar(20), @Valor float, @dtBase DateTime, @dtReajuste DateTime,
	    @SubTotal float, @Multa float, @Juros float, @Atualizacao float, @SeqCalcDe int, @SeqCalcAte int, @Intervalo bit,
	    @Texto varchar(1000), @SubTexto varchar(1000), @TipoPessoa bit, @TipoDebito bit, @PodeParar bit, @ValorTotal float,
	    @PercAtualiz float, @PercMulta float, @PercJuros float, @DataFixa bit, @Cont int, @IdProcedimentoT int, @Em_Uso bit

IF @DataCalculo IS NULL 
	SET @DataCalculo = GETDATE()
CREATE TABLE #tmpSubTotais(NumSequencia int, Valor float, Percentual float, RotinaCalculo int)
...
SET @RetornoMulta = @Multa 
SET @RetornoAtualizacao = @Atualizacao 
SET @RetornoValorTotal  = @ValorTotal

drop table #tmpSubTotais
drop table #tmpErrosCalculo
