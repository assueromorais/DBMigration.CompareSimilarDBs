...
* 
*/ 
CREATE  FUNCTION [dbo].[FormataTelefoneCelular_Unico]
(
	@UF        VARCHAR(2),
...
BEGIN
	...
	BEGIN
	    ...
	    BEGIN
	        ...
	        BEGIN
	            SET @DDD = (
	                    ...
	                    FROM   @Tab_DDD
	                    WHERE  UF = @UF
	                           AND DDD IN (11, 21, 27, 31, 41, 47, 51, 61, 62, 63, 65, 67, 68, 69, 71, 77, 79, 81, 82, 83, 84, 85, 86, 87, 91, 92, 95, 96, 97, 98)
	                )
	        END
	    ...
	    --IF LEN(@TelefonePuro) > 10
	    --BEGIN
	    --    SET @TelefonePuro = LTRIM(RTRIM(LEFT (@TelefonePuro, 10)))
	    --END
	    ...
	    BEGIN
	        ...
	        BEGIN
	            ...
	            IF dbo.ValidaTelefoneCelular(@DDD + '-' + @TelefonePuro) = 1
	            BEGIN
	...
	IF (LEN(@TelefoneFormatado) = 11) OR (LEN(@TelefoneFormatado) = 12)
