...
CREATE FUNCTION [dbo].[AtualizaDebitosAll]
(
	@DataVcto      DATETIME,
...
RETURNS @result TABLE (
            ...
            PercAtualiz DECIMAL(10, 2),
            PercMulta DECIMAL(10, 2),
            PercJuros DECIMAL(10, 2),
            IdProcedimento INT,
            CodErro INT,
            AlertaINPC BIT
        )
	...
	IF @CodErro = 0
	    ...
	    BEGIN
	        SELECT TOP 1 @NumSeq = NumSequenciaCalculo,
	               ...
	               @AplicarSobreCalcSeq = AplicarSobreCalculoSequencia,
	        ...
	        IF (
	           ...
	           AND /*DM13871 o sinal estava menor (inserido sinal de =)*/ 
	               (
	                   @DiaFim IS NULL
	                   OR DATEDIFF(DAY, @DataVcto, @DataCalculo) <= @DiaFim
	               )
	           AND /*DM13871 o sinal estava maior (inserido sinal de =)*/ 
	               (@DataFim >= @DataVcto)
...
/*Oc.130059_FIM_********************************************************************************************************************/ 			    	            
	...
	BEGIN
	    ...
	    FROM   @Subtotais
	    WHERE  RotinaCalculo IN (2, 10, 14)
	    
	    SELECT @Atualizacao = ISNULL(SUM(Valor), 0)
	    FROM   @Subtotais
	    WHERE  RotinaCalculo NOT IN (1, 2, 10, 14, 15)/*DM13871- inserido o 15-MultaDiária*/  
	...
	/* PauloR em 19.08.04  
	INSERT INTO @result  
	SELECT @Multa, @Juros, @Atualizacao, @ValorTotal, (@Atualizacao / @ValorTotal * 100) as PercAtualiz,  
	(@Multa / @ValorTotal * 100) as PercMulta,  
	(@Juros / @ValorTotal * 100) as PercJuros,  
	ISNULL(@IdProcedimento, @Procedimento), @CodErro  
	*/ 

 	/*
	  OC. 190141 - Se o valor for menor que zero, retornamos 0. 
	  Se o valor da cotação do Índice for negativo, influencia nos campos abaixo
	*/
	...
	BEGIN
	    ...
	    SELECT @Multa,
	           ...
	           @Atualizacao,
	           @ValorTotal,
	           (@Atualizacao / @ValorTotal * 100) AS PercAtualiz,
	           (@Multa / @ValorTotal * 100) AS PercMulta,
	           (@Juros / @ValorTotal * 100) AS PercJuros,
	           ISNULL(@IdProcedimento, @Procedimento),
	           @CodErro,
	           @AlertaINPC
	END
	ELSE
