...
CREATE  FUNCTION [dbo].[Calc_Periodo](@IdDividaAtiva INT,@IncluiDebitosCancelados BIT) RETURNS VARCHAR (250) AS
BEGIN
/*Declaração de Variáveis=====================================================*/
  ...
  IF @IncluiDebitosCancelados = 1
  INSERT INTO @Periodo(AnoRef)  SELECT DISTINCT CAST((Year(DataReferencia)) AS CHAR (4) )  FROM DebitosDividaAtiva  WHERE IdDividaAtiva = @IdDividaAtiva   ORDER BY 1 ASC	
  ELSE 
   INSERT INTO @Periodo(AnoRef)	SELECT DISTINCT CAST((Year(DataReferencia)) AS CHAR (4) )   FROM DebitosDividaAtiva   WHERE IdDividaAtiva = @IdDividaAtiva  AND SituacaoDebito = 1  ORDER BY 1 ASC 
