CREATE FUNCTION [dbo].[Calc_INPC] (@Valor float, @DataVencimento datetime, @DataCalculo datetime, @DataFixa bit)
	...
	BEGIN
	  ...
	  IF (SELECT COUNT(*) FROM Indices WHERE IdIndice = 4 AND Data >= @DataVencimento AND Data < @DataCalculo) =
